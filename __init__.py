...
if session is None or str(session).strip() == "" or str(session).lower() == "none":
    session = "default"
else:
...
def _session_data(session_name):
    return mod_nx_session.get(session_name) or {}

def _require_session(session_name):
    data = _session_data(session_name)
    if not data.get("base_url"):
        raise RuntimeError(
            f"Session '{session_name}' not initialized. "
